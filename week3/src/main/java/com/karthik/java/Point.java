...
public class Point implements Comparable<Point> {
	...
	public double slopeTo(Point that) {
		...
		double slope = (deltaX == 0.0) ? ((deltaY == 0.0) ? Double.NEGATIVE_INFINITY : Double.POSITIVE_INFINITY)
				: (deltaY == 0.0 ? +0.0 : deltaY / deltaX);
		return slope;
	}
	...
	public int compareTo(Point that) {
		Integer deltaY = new Integer(this.y).compareTo(that.y);
		return deltaY == 0 ? new Integer(this.x).compareTo(that.x) : deltaY;
	}
	...
	public Comparator<Point> slopeOrder() {
		return new Comparator<Point>() {
			...
			public int compare(Point o1, Point o2) {
				return new Double(Point.this.slopeTo(o1)).compareTo(Point.this.slopeTo(o2));
			}
	...
	public static void main(String[] args) {
		...
		Point[] input = { new Point(5, 1), new Point(4, 1), new Point(3, 1), new Point(2, 1), new Point(1, 1) };
		Arrays.sort(input);
		System.out.println("Sorted Points Array : " + Arrays.asList(input));
	}
}