...
public class Maze2 {
    ...
    private void backTrack(int[][] maze, int[] start, int[] destination) {
        if (start[0] < 0 || start[0] >= maze.length) return;
    ...
    public int shortestDistance(int[][] maze, int[] start, int[] destination) {
        memo = new Integer[maze.length][maze[0].length];
        backTrack(maze, start, destination);
    ...
    public static void main(String[] args) {
        ...
        int[] start = {0, 4};
        int[] end = {3, 2};
        System.out.println("Maze Traversal Result : " + new Maze2().shortestDistance(maze, start, end));
    }
}
